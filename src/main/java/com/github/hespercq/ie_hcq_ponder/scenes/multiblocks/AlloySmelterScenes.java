...
public class AlloySmelterScenes {

  public static void forming(SceneBuilder builder, SceneBuildingUtil util) {
    final Mods IE = Mods.IE;

    final String sceneId = "alloy_smelter/forming";
    final String sceneTitle = "Forming the Alloy Kiln Multiblock";
    final ResourceLocation multiBlockRL = IE.rl("multiblocks/alloy_smelter");

    final Selection multiBlockSelection = util.select().fromTo(1, 3, 1, 2, 4, 2);
    final Vec3 multiBlockOffset = util.vector().of(0, -2, 0);

    final BlockPos templateOrigin = util.grid().at(1, 1, 1);
    final int basePlateSize = 4;
