...
public class CokeOvenScenes {
        public static void forming(SceneBuilder builder, SceneBuildingUtil util) {
                ...
                BlockPos coke_oven_template_origin = util.grid().at(1, 1, 1);
                scene.idle(20);
                Selection coke_oven_template = scene.assembleMultiblockTemplate(IE.rl("multiblocks/coke_oven"),
                                coke_oven_template_origin, 30, 0);

                scene.world().replaceBlocks(coke_oven_template, Blocks.AIR.defaultBlockState(), true);
                ElementLink<WorldSectionElement> coke_oven_element = scene.world()
                                .showIndependentSectionImmediately(coke_oven);
